...
def highlight_cells(x, highlight_info=df_pic2.drop(columns=['CAB','NAMA PIC','SELISIH'])):
    ...
    for idx, row in highlight_info.iterrows():
        ...
        if row_index in df_styles.index and col_name in df_styles.columns:
            ...
            df_styles.at[row_index, col_name] = f'attr:🔴 {x.at[row_index, col_name]};'
...
df_pic.style.format(lambda x: format_number(x)).background_gradient(cmap='Reds', axis=1, subset=df_pic.columns[2:]).apply(highlight_cells, highlight_info=df_pic2.drop(columns=['CAB','NAMA PIC','SELISIH']), axis=None)
