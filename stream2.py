...
df_pic.iloc[:,1:] = df_pic.iloc[:,1:].fillna(0).astype(float).applymap(lambda x: f'{x:.0f}')
df_pic = df_pic.style.format(lambda x: '' if x==0 else x).background_gradient(cmap='Reds', axis=1, subset=df_pic.columns[1:])
