...
df_cab = pd.read_csv(f'Compile Merge (ALL).csv')
all_cab = st.multiselect('Pilih Cabang', df_cab['CAB'].unique())
...
if st.button('Show'):
        ...
        df_merge = pd.read_csv(f'Compile Merge (ALL).csv')
        df_breakdown = pd.read_csv(f'/Compile Breakdown (ALL).csv')
